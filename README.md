...
* **방법**:
  * 인식시키고자 하는 나만의 손 모양(가위, 바위, 보)을 정합니다.
  * 각 제스처당 **최소 50장 이상**의 사진을 촬영하여 다음 경로에 저장합니다:
    * `assets/dataset/rock/` (바위)
    * `assets/dataset/paper/` (보)
    * `assets/dataset/sissor/` (가위)
  * 다양한 각도, 거리, 배경에서 촬영할수록 모델의 인식률이 향상됩니다.
